...
Item = models.get_model("jellybaby", "item")

register = template.Library()

def jellyrender(parser, token):
    """
    Render a jellybaby ``Item`` by passing it through a snippet template.
    ...
    in this order:
        ...
        * ``jellybaby/snippets/<classname>.html``, where ``classname`` is the
          name of the specific item class (i.e. ``photo``).
          
        * ``jellybaby/snippets/item.html``.
        
    The template will be passed a context containing:
    
        ``item``
            The jellybaby ``Item`` object
...
class JellyrenderNode(template.Node):
    ...
    def render(self, context):
        ...
        template_list = [
            "jellybaby/snippets/%s.html" % type(object).__name__.lower(), 
            "jellybaby/snippets/item.html"
        ]
        if self.using:
            try:
...
def get_jellybaby_items(parser, token):
    """
    Load jellybaby ``Item`` objects into the context.In the simplest mode, the
    most recent N items will be returned.
    
    ::
    
        {# Fetch 10 most recent items #}
        {% get_jellybaby_items limit 10 as items %}
    ...
    unless you ask for them to be reversed::
        ...
        {% get_jellybaby_items limit 10 reversed as items %}
    
    You can also fetch items between two given dates::
    
        {% get_jellybaby_items between "2007-01-01" and "2007-01-31" as items %}
    ...
    Dates can also be the magic strings "now" or "today"::
        ...
        {% get_jellybaby_items between "2007-01-01" and "today" as items %}
    
    You can also limit items by type::
    
        {# Fetch the 10 most recent videos and photos #}
        {% get_jellybaby_items oftype video oftype photo limit 10 as items %}
    ...
    You can similarly exclude types using ``excludetype``::
        ...
        {% get_jellybaby_items excludetype video limit 10 as items %}
...
get_jellybaby_items = register.tag(get_jellybaby_items)
