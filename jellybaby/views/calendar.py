...
def year(request, year, queryset=None,
    ...
    Templates: ``jellybaby/calendar/year.html`` (default)
    Context:
        ``items``
        ...
        ``year``
            The year.
        ``previous``
            The previous year; ``None`` if that year was before jellybabying
            started..
        ``previous_link``
    ...
    previous = year - 1
    previous_link = urlresolvers.reverse("jellybaby.views.calendar.year", args=[previous])
    if previous < first.timestamp.year:
        previous = previous_link = None
    
    # And the next year
    next = year + 1
    next_link = urlresolvers.reverse("jellybaby.views.calendar.year", args=[next])
    if next > today.year:
        next = next_link = None
...
def month(request, year, month, queryset=None,
    template_name="jellybaby/calendar/month.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None):
    """
    ...
    Templates: ``jellybaby/calendar/month.html`` (default)
    Context:
        ``items``
        ...
        ``previous``
            The previous month; ``None`` if that month was before jellybabying
            started.
        ``previous_link``
    ...
    previous_link = urlresolvers.reverse("jellybaby.views.calendar.month", args=previous.strftime("%Y %b").lower().split())
    if previous < first.timestamp.date().replace(day=1):
        previous = None
    
    # And the next month
    next = last_day + datetime.timedelta(days=1)
    next_link = urlresolvers.reverse("jellybaby.views.calendar.month", args=next.strftime("%Y %b").lower().split())
    if next > today:
        next = None
...
def day(request, year, month, day, queryset=None, recent_first=False,
    template_name="jellybaby/calendar/day.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None):
    """
    ...
    Templates: ``jellybaby/calendar/day.html`` (default)
    Context:
        ``items``
        ...
        ``previous``
            The previous day; ``None`` if that day was before jellybabying
            started.
        ``previous_link``
    ...
    previous_link = urlresolvers.reverse("jellybaby.views.calendar.day", args=previous.strftime("%Y %b %d").lower().split())
    if previous < first.timestamp.date():
        previous = previous_link = None
    
    # And the next month
    next = day + datetime.timedelta(days=1)
    next_link = urlresolvers.reverse("jellybaby.views.calendar.day", args=next.strftime("%Y %b %d").lower().split())
    if next > today:
        next = next_link = None
